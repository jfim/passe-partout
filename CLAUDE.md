...
- **`app.py`** — defines all routes plus a background sweeper task started in the lifespan that calls `sweep_once()` every 30s to evict expired tabs through `BrowserPool.close_context`. `POST /fetch` is implemented as a thin wrapper that calls the same `create_tab` handler then deletes the tab in a `finally`. Both navigation entry points (`POST /tabs` and `POST /tabs/{id}/goto`) drive `Page.navigate` directly rather than nodriver's `tab.get()` (whose only added behavior is a flat 0.5s sleep — its real load-detection is commented out upstream); settling is left entirely to the client (e.g. `POST /tabs/{id}/wait`). Both accept an optional `referrer` field passed straight to `Page.navigate` (Chrome still applies the request's referrer policy, e.g. stripping it on an HTTPS→HTTP downgrade). `final_url` is taken from the captured main-document response (`nav.url`) before falling back to `tab.url`, since `tab.url` can still read `about:blank` immediately after navigate without the sleep.
